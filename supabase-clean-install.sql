...
DROP TABLE IF EXISTS public.viewed_fragments CASCADE;
...
-- 等待删除完成
SELECT '所有旧表已删除，3秒后开始创建...' as status;
SELECT pg_sleep(3);
...
CREATE INDEX idx_user_answers_created_at ON public.user_answers(created_at DESC);
...
CREATE INDEX idx_question_bank_chapter_day ON public.question_bank(chapter, day);
...
CREATE INDEX idx_answered_questions_user_id ON public.answered_questions(user_id);
CREATE UNIQUE INDEX idx_answered_questions_user_question
  ON public.answered_questions(user_id, question_text);

-- 6. 记忆碎片查看记录
CREATE TABLE public.viewed_fragments (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
...
CREATE INDEX idx_viewed_fragments_user_id ON public.viewed_fragments(user_id);
CREATE UNIQUE INDEX idx_viewed_fragments_user_fragment
